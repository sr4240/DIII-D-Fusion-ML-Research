...
def create_windows_with_shot_info(X, y, shots, window_size=150):
    ...
    all_shot_ids = np.array(all_shot_ids)

    print(f"Created {len(all_windows)} windows | Label distribution: {Counter(all_labels)}")
    print(f"Unique shots in windows: {len(np.unique(all_shot_ids))}")

    return all_windows, all_labels, all_shot_ids

def create_shot_based_split(all_windows, all_labels, all_shot_ids, train_ratio=0.7, val_ratio=0.15):
    """
    Split windows based on shots to prevent data leakage.
    """
    print("Creating shot-based train/val/test split...")
    
    # Get unique shots
    unique_shots = np.unique(all_shot_ids)
    np.random.seed(42)
    np.random.shuffle(unique_shots)
    
    # Calculate split indices
    n_shots = len(unique_shots)
    train_end = int(n_shots * train_ratio)
    val_end = int(n_shots * (train_ratio + val_ratio))
    
    train_shots = unique_shots[:train_end]
    val_shots = unique_shots[train_end:val_end]
    test_shots = unique_shots[val_end:]
    
    print(f"Train shots: {len(train_shots)} | Val shots: {len(val_shots)} | Test shots: {len(test_shots)}")
    
    # Create masks for each split
    train_mask = np.isin(all_shot_ids, train_shots)
    val_mask = np.isin(all_shot_ids, val_shots)
    test_mask = np.isin(all_shot_ids, test_shots)
...
def create_windows_and_split(X, y, shots, window_size=150):
    """
    Create windows and split them using shot-based splitting to prevent data leakage.
    """
    # Create windows with shot information
    all_windows, all_labels, all_shot_ids = create_windows_with_shot_info(X, y, shots, window_size)
    ...
    # Shot-based split
    train_windows, train_labels, val_windows, val_labels, test_windows, test_labels = create_shot_based_split(
        all_windows, all_labels_mapped, all_shot_ids
    )
...
def train_and_evaluate(train_loader, val_loader, test_loader, n_features):
    ...
    for epoch in range(n_epochs):
        ...
        if train_batches > 0 and val_batches > 0:
            ...
            if val_accuracy > best_val_accuracy:
                ...
                torch.save(model.state_dict(), 'best_improved_plasma_classifier_shot_based.pth')
                patience_counter = 0
            else:
                patience_counter += 1
        else:
    ...
    if len(train_losses) > 0:
        ...
        ax1.set_ylabel('Loss')
        ax1.set_title('Training and Validation Loss (Shot-Based Split)')
        ax1.legend()
        ...
        ax2.set_title('Training and Validation Accuracy (Shot-Based Split)')
        ax2.legend()
        ax2.grid(True, alpha=0.3)

        plt.tight_layout()
        plt.savefig('training_curves_shot_based.png', dpi=800, bbox_inches='tight')
        plt.show()
    ...
    model.load_state_dict(torch.load('best_improved_plasma_classifier_shot_based.pth'))
    model.eval()
    ...
    # Results
    print("\nClassification Report (Shot-Based Split):")
    ...
    plt.title('Normalized Confusion Matrix (Shot-Based Split)')
    plt.ylabel('True Label')
    plt.xlabel('Predicted Label')
    plt.savefig('confusion_matrix_shot_based.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.title('ROC Curves (Shot-Based Split)')
    plt.legend()
    plt.savefig('roc_curves_shot_based.png', dpi=300, bbox_inches='tight')
    plt.show()

def main():
    print("=== Improved Plasma Four-State Classification (Shot-Based Split) ===")
