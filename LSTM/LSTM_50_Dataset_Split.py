...
class LSTMFirstNN(nn.Module):
    ...
    Binary classification: Suppressed (0) vs Dithering/Mitigated/ELMing (1).
    """
    def __init__(self, n_features, n_classes=2, lstm_hidden=64, nn_hidden_sizes=[128, 64]):
        ...
        print(f"{'='*60}")
        print(f"Architecture: LSTM (unidirectional) → NN → Classifier (Binary)")
...
def load_and_prepare_data():
    """Load and preprocess plasma data from two separate datasets"""
    print("Loading data from two datasets...")
    
    # Load LABEL_PROPAGATED_DATABASE.csv for train/val
    print("  Loading LABEL_PROPAGATED_DATABASE.csv...")
    df_train_val = pd.read_csv('/mnt/homes/sr4240/my_folder/LABEL_PROPAGATED_DATABASE.csv')
    print(f"    Loaded {len(df_train_val)} rows, {df_train_val['shot'].nunique()} unique shots")
    
    # Load plasma_data.csv for test
    print("  Loading plasma_data.csv...")
    df_test = pd.read_csv('/mnt/homes/sr4240/my_folder/plasma_data.csv')
    print(f"    Loaded {len(df_test)} rows, {df_test['shot'].nunique()} unique shots")
    
    # Remove overlapping shots from train_val to ensure test set purity
    test_shots = set(df_test['shot'].unique())
    overlap_shots = set(df_train_val['shot'].unique()) & test_shots
    if len(overlap_shots) > 0:
        print(f"  Removing {len(overlap_shots)} overlapping shots from train/val data")
        df_train_val = df_train_val[~df_train_val['shot'].isin(overlap_shots)]
        print(f"    Train/val after removing overlap: {len(df_train_val)} rows, {df_train_val['shot'].nunique()} unique shots")
    
    # Now tag the datasets
    df_train_val['dataset_source'] = 'train_val'
    df_test['dataset_source'] = 'test'
    
    # Combine datasets - using only common columns
    # Both datasets have the features we need: iln3iamp, betan, density, li, tritop, fs04_max_smoothed
    common_cols = ['shot', 'time', 'state', 'n', 'n_eped', 't_eped', 'p_eped', 'n_e', 
                   'betan', 'li', 'q95', 'Ip', 'bt0', 'bt', 'kappa', 'tribot', 'tritop', 
                   'dR_sep', 'zeff', 'rotation_edge', 'rotation_core', 'iun2iamp', 
                   'iln2iamp', 'iun3iamp', 'iln3iamp', 'iun2iphase', 'iln2iphase', 
                   'iun3iphase', 'iln3iphase', 'density', 'fs04', 'cerqtit1', 'pinj', 
                   'fs04_max_smoothed', 'fs_sum', 'fs_up_sum', 'dataset_source']
    
    # Select common columns
    df_train_val = df_train_val[[col for col in common_cols if col in df_train_val.columns]]
    df_test = df_test[[col for col in common_cols if col in df_test.columns]]
    
    # Combine
    df = pd.concat([df_train_val, df_test], ignore_index=True)
    print(f"\nCombined dataset: {len(df)} total rows")
    print(f"  Train/Val: {len(df[df['dataset_source'] == 'train_val'])} rows")
    print(f"  Test: {len(df[df['dataset_source'] == 'test'])} rows")

    # Remove problematic shot if present
    df = df[df['shot'] != 191675].copy()

    # Select only the specified 6 features
    # Note: Using fs04_max_smoothed (available in both datasets) instead of fs04_past_max_smoothed
    ...
    dataset_sources = dataset_sources[valid_mask]

    print(f"\nData shape after cleaning: {X.shape}")
    print(f"Label distribution (all data): {Counter(y)}")
    
    # Get train_val mask for fitting scaler
    train_val_mask = dataset_sources == 'train_val'
    print(f"\nTrain/Val data: {train_val_mask.sum()} points")
    print(f"Test data: {(~train_val_mask).sum()} points")

    # Fit StandardScaler ONLY on train_val data
    scaler = StandardScaler()
    X_scaled = np.zeros_like(X)
    X_scaled[train_val_mask] = scaler.fit_transform(X[train_val_mask])
    X_scaled[~train_val_mask] = scaler.transform(X[~train_val_mask])
    
    print("Scaler fitted on train_val data only")

    return X_scaled, y, times, shots, dataset_sources, selected_features, scaler

def create_windows_with_dataset_split(X, y, times, shots, dataset_sources, window_size=150, prediction_horizon_ms=50):
    """Create windows and split by dataset source - predicting state at future time
    
    This function splits data by dataset source:
    - train_val shots: randomly split into train (82.35%) and val (17.65%)
    - test shots: all go to test set
    ...
    - Suppressed (1) → 0
    - Dithering/Mitigated/ELMing (2, 3, 4) → 1
    """
    print(f"\nCreating windows of size {window_size} (predicting {prediction_horizon_ms}ms in the future)...")
    print("Binary classification: Suppressed (0) vs Dithering/Mitigated/ELMing (1)")
    print("Splitting by DATASET SOURCE (train_val vs test)")

    # Get unique shots by dataset source
    unique_shots = np.unique(shots)
    train_val_shots_mask = np.array([dataset_sources[shots == s][0] == 'train_val' for s in unique_shots])
    
    train_val_shots_list = unique_shots[train_val_shots_mask]
    test_shots_list = unique_shots[~train_val_shots_mask]
    
    print(f"\nDataset source split:")
    print(f"  Train/Val shots: {len(train_val_shots_list)}")
    print(f"  Test shots: {len(test_shots_list)}")

    # Randomly split train_val shots into train and val (82.35% / 17.65%)
    np.random.seed(42)
    shuffled_train_val = np.random.permutation(train_val_shots_list)
    
    train_size = int(0.8235 * len(train_val_shots_list))  # 82.35% for train
    
    train_shots = set(shuffled_train_val[:train_size])
    val_shots = set(shuffled_train_val[train_size:])
    test_shots = set(test_shots_list)
    ...
    # Binary label mapping: Suppressed (1) = 0, Dithering/Mitigated/ELMing (2,3,4) = 1
...
def train_model(model, train_loader, val_loader, device, n_epochs=50):
    ...
    for epoch in range(n_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), 'best_lstm_50ms_dataset_split_binary.pth')
            patience_counter = 0
...
def plot_results(train_losses, val_losses, train_accs, val_accs, all_preds, all_labels, class_names):
    ...
    plt.tight_layout()
    plt.savefig(f'lstm_{PREDICTION_HORIZON_MS}ms_dataset_split_binary_results.png', dpi=300, bbox_inches='tight')
    plt.show()

    print(f"Results saved to 'lstm_{PREDICTION_HORIZON_MS}ms_dataset_split_binary_results.png'")
...
def main():
    ...
    print("Classification: Binary (Suppressed vs Dithering/Mitigated/ELMing)")
    print("Split: BY DATASET SOURCE")
    print("  Train/Val: LABEL_PROPAGATED_DATABASE.csv")
    print("  Test: plasma_data.csv")
    print("=" * 60)
    ...
    X, y, times, shots, dataset_sources, features, scaler = load_and_prepare_data()

    # Create windows and split BY DATASET SOURCE
    train_X, train_y, val_X, val_y, test_X, test_y = create_windows_with_dataset_split(
        X, y, times, shots, dataset_sources, prediction_horizon_ms=PREDICTION_HORIZON_MS
    )
    ...
    # Load best model
    print("\nLoading best model...")
    model.load_state_dict(torch.load('best_lstm_50ms_dataset_split_binary.pth'))

    # Evaluate on test set (binary classification)
    ...
    print("\n" + "=" * 60)
    print(f"Training Complete! (Binary Classification, Predicting {PREDICTION_HORIZON_MS}ms into the future)")
    print("Dataset Split: Train/Val from LABEL_PROPAGATED, Test from plasma_data")
    print("=" * 60)
