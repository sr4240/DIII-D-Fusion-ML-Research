...
def create_windows_with_random_shot_split(X, y, shots, window_size=150):
    ...
    for shot_id in unique_shots:
        ...
        for i in range(len(shot_indices) - window_size + 1):
            ...
            if int(last_label) not in label_mapping:
                continue
...
def train_model(model, train_loader, val_loader, device, n_epochs=50):
    ...
    for epoch in range(n_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), 'best_lstm_last_point_random_shot.pth')
            patience_counter = 0
...
def plot_results(train_losses, val_losses, train_accs, val_accs, all_preds, all_labels, class_names):
    ...
    plt.tight_layout()
    plt.savefig('lstm_last_point_random_shot_results.png', dpi=300, bbox_inches='tight')
    plt.show()

    print("Results saved to 'lstm_last_point_random_shot_results.png'")
...
def main():
    ...
    # Load best model
    print("\nLoading best model...")
    model.load_state_dict(torch.load('best_lstm_last_point_random_shot.pth'))
