"""
Shot Visualization Script for Random Forest Analysis (Random Split Version)

This script visualizes Random Forest predictions for any shot in the plasma database.
It uses the same model parameters as Random_Forest_Random_Split.py and ensures
the specified shot is in the test set.

HOW TO USE:
1. Change the SHOT_NUMBER variable below to the shot you want to analyze
2. Run the script: python Shot_Visual_Analysis_Random_Split.py
...
# This shot will be guaranteed to be in the test set
SHOT_NUMBER = 169472
...
def load_and_train_model():
    """Load data and train the Random Forest model using the same parameters as Random_Forest_Random_Split.py"""
    ...
    # Select features and the target variable (same as Random_Forest_Random_Split.py)
    selected_features = ['iln3iamp', 'betan', 'density', 'n_eped', 'li', 'tritop', 'fs04_max_smoothed']
    target_column = 'state'
    
    # Clean the dataframe (same as Random_Forest_Random_Split.py)
    df_cleaned = df.dropna(subset=selected_features, how='any')
    df_cleaned = df_cleaned[df_cleaned['n'] == 3]
    
    # Ensure the target shot is in the test set
    print(f"\nEnsuring shot {SHOT_NUMBER} is in the test set...")
    
    # Check if shot exists
    if not (df_cleaned['shot'] == SHOT_NUMBER).any():
        raise ValueError(f"Shot {SHOT_NUMBER} not found in the cleaned dataset")
    
    # Separate the target shot from the rest of the data
    shot_mask = df_cleaned['shot'] == SHOT_NUMBER
    shot_data = df_cleaned[shot_mask].copy()
    other_data = df_cleaned[~shot_mask].copy()
    
    print(f"Target shot {SHOT_NUMBER}: {len(shot_data)} data points")
    print(f"Other data: {len(other_data)} data points")
    
    # Prepare input (X) and target (y) for non-shot data
    X_other = other_data[selected_features]
    y_other = other_data[target_column]
    
    # Split the other data into training and testing sets (same random state as Random_Forest_Random_Split.py)
    X_train, X_test_other, y_train, y_test_other, idx_train, idx_test_other = train_test_split(
        X_other, y_other, other_data['original_index'], test_size=0.2, random_state=43)
    
    # Add the target shot to the test set
    X_test_shot = shot_data[selected_features]
    y_test_shot = shot_data[target_column]
    
    # Combine test sets
    X_test = pd.concat([X_test_other, X_test_shot], ignore_index=True)
    y_test = pd.concat([y_test_other, y_test_shot], ignore_index=True)
    
    print(f"Training set size: {len(X_train)}")
    print(f"Test set size: {len(X_test)} (including {len(X_test_shot)} points from shot {SHOT_NUMBER})")
    
    # Train Random Forest Classifier (same parameters as Random_Forest_Random_Split.py)
    print("Training Random Forest model...")
    clf = RandomForestClassifier(n_estimators=500, max_depth=35, min_samples_split=2, 
                                min_samples_leaf=1, max_features='sqrt', random_state=43, n_jobs=-1)
    ...
    return clf, selected_features, df_cleaned, shot_data
...
def extract_shot_data(df, shot_number=169472):
...
def make_predictions_on_shot(clf, shot_data, selected_features):
    ...
    print(f"Prediction accuracy for shot {shot_data['shot'].iloc[0]}: {shot_accuracy:.4f}")
...
def create_comprehensive_visualization(shot_data):
    ...
    fig.suptitle(f'Plasma State Classification Analysis for Shot {shot_data["shot"].iloc[0]} - Random Forest (Split Based on Shot)', 
    ...
    # Plot 3: fs04 time series with correct/incorrect predictions overlay
    ax3 = axes[2]
    ...
    # Add markers for prediction confidence (no outline, no colorbar)
...
def create_detailed_analysis_plot(shot_data):
    ...
    fig.suptitle(f'Shot {shot_data["shot"].iloc[0]} Detailed Analysis - Random Forest (Split Based on Shot Number)', fontsize=20, fontweight='bold')
...
def print_detailed_statistics(shot_data):
    ...
    for state in [1, 2, 3, 4]:
        ...
        if state_mask.any():
            ...
            print(f"  {state_name} (State {state}): {state_correct}/{state_total} ({state_accuracy:.4f})")
    
    # Prediction confidence statistics
    print(f"\nPrediction Confidence Statistics:")
    print(f"  Mean confidence: {shot_data['prediction_confidence'].mean():.4f}")
    print(f"  Std confidence: {shot_data['prediction_confidence'].std():.4f}")
    print(f"  Min confidence: {shot_data['prediction_confidence'].min():.4f}")
    print(f"  Max confidence: {shot_data['prediction_confidence'].max():.4f}")
    ...
    print(f"\nConfidence by Prediction Accuracy:")
    print(f"  Correct predictions - Mean: {correct_confidence.mean():.4f}, Std: {correct_confidence.std():.4f}")
    print(f"  Incorrect predictions - Mean: {incorrect_confidence.mean():.4f}, Std: {incorrect_confidence.std():.4f}")
...
def main():
    ...
    print(f"Starting Shot {SHOT_NUMBER} Visualization Analysis (Split Based on Data Point)")
    print("="*60)
    
    # Load data and train model (ensures shot is in test set)
    clf, selected_features, df_cleaned, shot_data_preloaded = load_and_train_model()
    
    # Extract shot data (already extracted in load_and_train_model, but we'll use it)
    shot_data = shot_data_preloaded.sort_values('time').reset_index(drop=True)
