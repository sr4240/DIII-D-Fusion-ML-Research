...
def select_features_and_clean(df):
    ...
    def map_states_to_binary(state):
        if state in [1]:  # Suppressed, Dithering, Mitigated -> Suppressed
            return 0
        elif state == 4:  # ELMing
            return 1
        else:
...
def main():
    """
    Main execution function for advanced plasma parameter analysis with random splitting
    """
    print("=== Advanced Plasma Parameter Space Exploration (Random Split) ===")
    print("Using ML-assisted sampling and adaptive scenarios with random data splitting\n")
    ...
    if not os.path.exists(MODEL_FILE) or not os.path.exists(FEATURES_FILE):
        ...
        # Prepare random data splits
        X_train, X_test, y_train, y_test = prepare_random_splits(df_cleaned, available_features)
