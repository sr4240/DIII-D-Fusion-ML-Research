...
def find_transition_borders(df, from_state=1, to_state=4, time_window=100):
    """
    Find borders between suppressed (1) and ELMing (4) states

    Args:
        df: DataFrame with 'shot', 'time', 'state' columns
        from_state: Initial state (1 = suppressed)
        to_state: Final state (4 = ELMing)
    ...
    for shot_id in df['shot'].unique():
        ...
        for i, idx in enumerate(change_indices):
            ...
            # Check if this is a suppressed → ELMing transition
...
def calculate_variable_deltas(df, transitions, variables, time_window=60):
    ...
    Args:
        ...
        time_window: Time window in milliseconds (80ms default)
...
# Find transitions
print("Finding suppressed → ELMing transitions...")
transitions = find_transition_borders(df_cleaned, from_state=1, to_state=4)
