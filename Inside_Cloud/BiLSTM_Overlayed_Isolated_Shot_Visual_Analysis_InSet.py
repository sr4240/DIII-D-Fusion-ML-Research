...
The shot is included in the training data (mixed with all other data).

HOW TO USE:
1. Change the SHOT_NUMBER variable below to the shot you want to analyze
2. Run the script: python BiLSTM_Overlayed_Isolated_Shot_Visual_Analysis_InSet.py
...
OUTPUT_PREFIX = f"bilstm_overlayed_shot_{SHOT_NUMBER}_inset"
...
def load_and_prepare_data(shot_number=SHOT_NUMBER):
    """Load data and prepare it (shot is included in training data)"""
    ...
    # IMPORTANT: Include the target shot in training data
    print(f"Including shot {shot_number} in training data...")
    df_training = df_cleaned.copy()  # Use all data including target shot
    df_target_shot = df_cleaned[df_cleaned['shot'] == shot_number].copy()

    print(f"Training data: {len(df_training)} rows (including shot {shot_number})")
    print(f"Target shot data: {len(df_target_shot)} rows (shot {shot_number})")
...
def train_bilstm_model(df_training, selected_features, device):
    """Train the BiLSTM model on all data (including target shot)"""
    print("Training BiLSTM model on all data (including target shot)...")
...
def create_overlayed_visualization(shot_data):
    ...
    fig.suptitle(f'Plasma State Classification Analysis for Shot {SHOT_NUMBER} - BiLSTM (Split Based on Data Point)',
...
def main():
    ...
    print(f"Starting BiLSTM Overlayed Shot {SHOT_NUMBER} Visualization Analysis...")
    print("="*70)
    ...
    # Load data (shot is included in training data)
    df_cleaned, df_training, df_target_shot, selected_features = load_and_prepare_data(SHOT_NUMBER)

    # Train BiLSTM model (including target shot)
