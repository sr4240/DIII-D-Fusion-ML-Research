"""
ALL Shots Isolated Shot Visualization Script with BiLSTM Classification (REVERSE ORDER)

This script iterates through ALL unique shot numbers in the dataset IN REVERSE ORDER and generates BiLSTM predictions
...
HOW TO USE:
1. Run the script: python ALL_BiLSTM_Overlayed_Isolated_Shot_Visual_Analysis_Reverse.py
2. The script will automatically process all unique shots in the dataset in reverse order (highest to lowest)
3. For each shot, it will generate visualization files showing BiLSTM predictions

The script provides a comprehensive view of model performance across all shots.
"""
...
def main():
    """Main function to run the complete analysis for ALL shots in REVERSE order"""
    print("="*70)
    print("Starting BiLSTM Overlayed Analysis for ALL SHOTS (REVERSE ORDER)")
    print("="*70)
    ...
    # Get all unique shots in REVERSE order (highest to lowest)
    unique_shots = sorted(df_cleaned['shot'].unique(), reverse=True)
    total_shots = len(unique_shots)

    print(f"\nFound {total_shots} unique shots to process in REVERSE order")
    ...
    # Final summary
    print("\n" + "="*70)
    print("ALL SHOTS PROCESSING COMPLETE")
    print("="*70)
