"""
ALL Shots Isolated Shot Visualization Script with BiLSTM Classification (40% to 60% Range)

This script processes unique shot numbers from the 40% to 60% range of the dataset and generates BiLSTM predictions
for each shot. Each shot is completely excluded from the training data when being analyzed to provide a realistic assessment.

HOW TO USE:
1. Run the script: python ALL_BiLSTM_Overlayed_Isolated_Shot_Visual_Analysis_40_60.py
2. The script will automatically process shots from the 40% to 60% range of unique shots
3. For each shot, it will generate visualization files showing BiLSTM predictions

The script provides a comprehensive view of model performance across the middle range of shots.
"""
...
def load_and_prepare_data_excluding_shot(exclude_shot_number):
...
def create_overlayed_visualization(shot_data, shot_number):
    """Create comprehensive visualization"""
    print("Creating visualization...")

    # Create figure with subplots
    fig, axes = plt.subplots(3, 1, figsize=(24, 18))
    fig.suptitle(f'Isolated Shot {shot_number} Analysis with BiLSTM Classification',
...
def process_single_shot(shot_number, df_cleaned, selected_features, device, output_dir):
    """Process a single shot and return its results"""
    print("\n" + "#"*70)
    ...
    try:
        ...
        df_cleaned_shot, df_training, df_target_shot, selected_features = load_and_prepare_data_excluding_shot(shot_number)

        # Train BiLSTM model (excluding target shot)
        model, feature_scaler = train_bilstm_model(df_training, selected_features, device)

        # Check if shot exists
        if not check_shot_exists(df_cleaned_shot, shot_number):
            print(f"Skipping shot {shot_number} - not found in dataset")
            return None

        # Extract shot data
        shot_data = extract_shot_data(df_cleaned_shot, shot_number=shot_number)

        # Make BiLSTM predictions
        shot_data = make_predictions_on_shot(model, shot_data, selected_features, feature_scaler, device)

        # Create visualizations
        fig1 = create_overlayed_visualization(shot_data, shot_number)
        ...
        print(f"\nERROR processing shot {shot_number}: {str(e)}")
        import traceback
        traceback.print_exc()
        return None

def main():
    """Main function to run the complete analysis for shots in 40%-60% range"""
    print("="*70)
    print("Starting BiLSTM Overlayed Analysis for SHOTS (40% to 60% Range)")
    print("="*70)
    ...
    # Load data once to get all unique shots
    print("Loading plasma data to identify all unique shots...")
    possible_paths = [
        '../plasma_data.csv',
        'plasma_data.csv',
    ...
    all_unique_shots = sorted(df_cleaned['shot'].unique())
    total_all_shots = len(all_unique_shots)

    # Calculate 40% to 60% range
    start_idx = int(total_all_shots * 0.40)
    end_idx = int(total_all_shots * 0.60)

    # Select shots in the 40%-60% range
    unique_shots = all_unique_shots[start_idx:end_idx]
    total_shots = len(unique_shots)

    print(f"\nFound {total_all_shots} total unique shots in dataset")
    print(f"Processing 40%-60% range: indices {start_idx} to {end_idx-1}")
    print(f"This corresponds to {total_shots} shots to process")
    print(f"Shots to process: {unique_shots}\n")
    ...
    for idx, shot_number in enumerate(unique_shots, 1):
        print(f"\n{'='*70}")
        print(f"Processing shot {idx}/{total_shots}: {shot_number}")
        print(f"{'='*70}")

        result = process_single_shot(shot_number, df_cleaned, selected_features, device, output_dir)

        if result is not None:
            results[shot_number] = result
            successful_shots += 1
        else:
            failed_shots += 1

        print(f"\nProgress: {idx}/{total_shots} shots processed ({successful_shots} successful, {failed_shots} failed)")

    # Final summary
    print("\n" + "="*70)
    print("SHOTS (40%-60% RANGE) PROCESSING COMPLETE")
    print("="*70)
    print(f"Total shots in range: {total_shots}")
    print(f"Successful: {successful_shots}")
