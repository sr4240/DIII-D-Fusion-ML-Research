"""
Overlayed Isolated Shot Visualization Script with kNN Cloud Classification
...
2. Run the script: python Overlayed_Isolated_Shot_Visual_Analysis.py
...
OUTPUT_PREFIX = f"overlayed_shot_{SHOT_NUMBER}"
...
def load_and_train_model_excluding_shot(exclude_shot_number = SHOT_NUMBER):
...
def create_overlayed_visualization(shot_data):
    ...
    fig.suptitle(f'Isolated Shot {SHOT_NUMBER} Analysis with kNN Cloud Classification',
    ...
    # Add markers for prediction confidence
    ...
    if valid_knn.any():
        ...
        # Estimate threshold from in/out cloud classification
    ...
    ax4.set_title('Distance from Training Data Cloud with Prediction Accuracy', fontsize=18, fontweight='bold')
...
def main():
    ...
    print(f"Starting Overlayed Isolated Shot {SHOT_NUMBER} Visualization Analysis...")
    print("="*70)
    print(f"IMPORTANT: Shot {SHOT_NUMBER} will be EXCLUDED from training data")
    print("="*70)

    # Load data and train Random Forest model (excluding target shot)
    clf, rf_selected_features, df_cleaned, df_target_shot, df_training = load_and_train_model_excluding_shot(SHOT_NUMBER)
