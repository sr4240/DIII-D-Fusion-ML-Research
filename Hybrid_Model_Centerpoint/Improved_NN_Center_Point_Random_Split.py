...
class ImprovedPlasmaNN(nn.Module):
    def __init__(self, n_features, window_size=150, n_classes=4):
        super().__init__()

        input_size = n_features * window_size  # Flattened input

        # Reduced feedforward network with residual connections
        self.input_projection = nn.Linear(input_size, 256)  # Reduced from 512

        # Smaller dense blocks with skip connections
        ...
        # Projection for LSTM input
        self.lstm_projection = nn.Linear(128, 128)  # Reduced from 256
        ...
        self.attention = MultiHeadAttention(hidden_dim=128, num_heads=4)  # Reduced from 256/8

        # Smaller classifier
        self.classifier = nn.Sequential(
            nn.Linear(128 + 128, 128),  # Reduced from 512->256
            nn.ReLU(),
            nn.Dropout(0.5),
            nn.Linear(128, 64),  # Reduced from 128
            nn.ReLU(),
            nn.Dropout(0.4),
            nn.Linear(64, n_classes)  # Reduced from 128
        )
    ...
    def forward(self, x):
        ...
        # Dense block 1 with residual connection
        residual = x
        x = self.dense_block1(x)
        x = nn.functional.relu(x + residual)

        # Dense block 2 with projection for dimension change
        x_proj = nn.functional.adaptive_avg_pool1d(x.unsqueeze(1), 128).squeeze(1)
        x = self.dense_block2(x)
        x = nn.functional.relu(x + x_proj)

        # Store NN features for later concatenation
        nn_features = x  # Now 128-dim

        # Prepare for LSTM processing
        x = self.lstm_projection(x)
        x = x.unsqueeze(1).repeat(1, 10, 1)  # Create sequence for LSTM

        # LSTM processing
        x_lstm, _ = self.lstm(x)  # (batch_size, seq_len, 128)

        # Multi-head attention
        x_attn = self.attention(x_lstm)  # (batch_size, seq_len, 128)
        x_attn_pooled = torch.mean(x_attn, dim=1)  # (batch_size, 128)

        # Concatenate NN features with attention output
        x_combined = torch.cat([nn_features, x_attn_pooled], dim=1)
...
def train_and_evaluate(train_loader, val_loader, test_loader, n_features, window_size=150):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Using device: {device}")

    # Model setup with improved NN architecture
    model = ImprovedPlasmaNN(n_features=n_features, window_size=window_size, n_classes=4).to(device)
    ...
    for epoch in range(n_epochs):
        ...
        if train_batches > 0 and val_batches > 0:
            ...
            if val_accuracy > best_val_accuracy:
                ...
                torch.save(model.state_dict(), 'best_improved_nn_classifier.pth')
                patience_counter = 0
            else:
                patience_counter += 1
        else:
    ...
    if len(train_losses) > 0:
        ...
        ax1.set_ylabel('Loss')
        ax1.set_title('Training and Validation Loss (NN)')
        ax1.legend()
        ...
        ax2.set_title('Training and Validation Accuracy (NN)')
        ax2.legend()
        ax2.grid(True, alpha=0.3)

        plt.tight_layout()
        plt.savefig('training_curves_improved_nn.png', dpi=800, bbox_inches='tight')
        plt.show()
    ...
    model.load_state_dict(torch.load('best_improved_nn_classifier.pth'))
    model.eval()
    ...
    plt.title('Normalized Confusion Matrix (Improved NN)')
    plt.ylabel('True Label')
    plt.xlabel('Predicted Label')
    plt.savefig('confusion_matrix_improved_nn.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.title('ROC Curves (Improved NN)')
    plt.legend()
    plt.savefig('roc_curves_improved_nn.png', dpi=300, bbox_inches='tight')
    plt.show()

def main():
    print("=== Improved Plasma Four-State Classification (NN) ===")
