...
class SimpleLSTMNN(nn.Module):
    """
    A simplified hybrid model combining Neural Network (NN) for feature extraction
    and LSTM for temporal pattern recognition.
    """
    def __init__(self, n_features, n_classes=4, lstm_hidden=64, nn_hidden_sizes=[128, 256, 128]):
        super(SimpleLSTMNN, self).__init__()

        # Simple NN layers for feature extraction (replaces CNN)
        self.nn_layers = nn.ModuleList()

        # Build the NN feature extractor
        input_dim = n_features
        for hidden_size in nn_hidden_sizes:
            self.nn_layers.append(nn.Sequential(
        ...
        # Final NN output dimension
        self.nn_output_dim = nn_hidden_sizes[-1]

        # Simple LSTM layer
        self.lstm = nn.LSTM(
            input_size=self.nn_output_dim,
        ...
        self.feature_agg = nn.Sequential(
            nn.Linear(self.nn_output_dim, self.nn_output_dim // 2),
            nn.ReLU(),
            nn.BatchNorm1d(self.nn_output_dim // 2)
        )

        # Simple classifier head
        self.classifier = nn.Sequential(
            nn.Linear(lstm_hidden * 2 + self.nn_output_dim // 2, 128),
            nn.ReLU(),
            nn.Dropout(0.3),
        ...
        print(f"Simple LSTM-NN Model initialized with {total_params:,} parameters")

    def forward(self, x):
        # x shape: (batch_size, n_features, sequence_length)
        batch_size, n_features, seq_len = x.shape

        # Transpose to (batch_size, sequence_length, n_features) for NN processing
        x_transposed = x.transpose(1, 2)  # (batch_size, seq_len, n_features)

        # Process each time step through NN
        nn_outputs = []
        for t in range(seq_len):
            # Extract features at time t
            x_t = x_transposed[:, t, :]  # (batch_size, n_features)

            # Pass through NN layers
            for layer in self.nn_layers:
                x_t = layer(x_t)

            nn_outputs.append(x_t)

        # Stack NN outputs: (batch_size, seq_len, nn_output_dim)
        nn_features = torch.stack(nn_outputs, dim=1)

        # LSTM processing
        lstm_output, _ = self.lstm(nn_features)  # (batch_size, seq_len, lstm_hidden*2)

        # Take the last LSTM output
        lstm_features = lstm_output[:, -1, :]  # (batch_size, lstm_hidden*2)

        # Aggregate NN features across sequence
        nn_pooled = torch.mean(nn_features, dim=1)  # (batch_size, nn_output_dim)
        nn_aggregated = self.feature_agg(nn_pooled)  # (batch_size, nn_output_dim // 2)

        # Concatenate LSTM and NN features
        combined_features = torch.cat([lstm_features, nn_aggregated], dim=1)
...
def load_and_prepare_data():
    ...
    # Select important features (same as original)
    important_features = ['n_eped', 'li', 'q95', 'bt0', 'kappa', 'tribot',
                         'tritop', 'dR_sep', 'fs04_max_smoothed', 'iln3iamp',
                         'fs04_max_avg', 'fs_sum', 'fs_up_sum']
...
def train_model(model, train_loader, val_loader, device, n_epochs=50):
    ...
    for epoch in range(n_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), 'best_simple_lstm_nn.pth')
            patience_counter = 0
        else:
            patience_counter += 1
...
def plot_results(train_losses, val_losses, train_accs, val_accs, all_preds, all_labels, class_names):
    ...
    plt.tight_layout()
    plt.savefig('simple_lstm_nn_results.png', dpi=300, bbox_inches='tight')
    plt.show()

    print("Results saved to 'simple_lstm_nn_results.png'")

def main():
    """Main training pipeline"""
    print("=" * 50)
    print("Simple LSTM-NN Hybrid Model for Plasma Classification")
    print("=" * 50)
    ...
    # Create model
    model = SimpleLSTMNN(n_features=len(features), n_classes=4).to(device)
    ...
    # Load best model
    print("\nLoading best model...")
    model.load_state_dict(torch.load('best_simple_lstm_nn.pth'))
