...
class ParallelLSTMCNN(nn.Module):
    """
    A parallel hybrid model where LSTM and CNN process raw data independently,
    then their features are concatenated for classification.

    Architecture:
    Raw Data → LSTM → LSTM Features ↘
                                      → Concatenate → Classifier
    Raw Data → CNN  → CNN Features  ↗
    """
    def __init__(self, n_features, n_classes=4, lstm_hidden=64, cnn_channels=32):
        super(ParallelLSTMCNN, self).__init__()

        # CNN branch for raw data feature extraction
        ...
        # LSTM branch for raw data temporal processing
        # LSTM directly processes the raw features, not CNN output
        self.lstm = nn.LSTM(
            input_size=n_features,  # Changed: Now uses raw features directly
            hidden_size=lstm_hidden,
            num_layers=2,  # Added more layers for better temporal modeling
            batch_first=True,
            bidirectional=True,
            dropout=0.2  # Added dropout between LSTM layers
        )

        # Global average pooling for CNN features
        self.global_avg_pool = nn.AdaptiveAvgPool1d(1)

        # Attention mechanism for LSTM outputs (optional enhancement)
        self.lstm_attention = nn.Sequential(
            nn.Linear(lstm_hidden * 2, 1),
            nn.Softmax(dim=1)
        )

        # Classifier head combines both LSTM and CNN features
        self.classifier = nn.Sequential(
            nn.Linear(lstm_hidden * 2 + self.cnn_output_dim, 256),
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(256, 128),
            nn.ReLU(),
        ...
        print(f"\nParallel LSTM-CNN Model initialized")
        print(f"{'='*60}")
        print(f"Architecture: Raw Data → [LSTM || CNN] → Concatenate → Classify")
    ...
    def forward(self, x):
        ...
        # ========== PARALLEL BRANCH 1: CNN Processing ==========
        # CNN processes raw data directly
        ...
        # ========== PARALLEL BRANCH 2: LSTM Processing ==========
        # LSTM processes raw data directly (not CNN output!)
        # Transpose for LSTM: (batch_size, seq_len, n_features)
        lstm_input = x.transpose(1, 2)

        # LSTM processing on raw features
        lstm_output, (hidden, cell) = self.lstm(lstm_input)  # (batch_size, seq_len, lstm_hidden*2)

        # Apply attention to LSTM outputs for better temporal aggregation
        attention_weights = self.lstm_attention(lstm_output)  # (batch_size, seq_len, 1)
        lstm_attended = torch.sum(lstm_output * attention_weights, dim=1)  # (batch_size, lstm_hidden*2)

        # Alternative: Just use the last LSTM output (comment out attention if not needed)
        # lstm_features = lstm_output[:, -1, :]  # (batch_size, lstm_hidden*2)

        # ========== FUSION: Concatenate Features ==========
        # Concatenate LSTM and CNN features from parallel branches
        combined_features = torch.cat([lstm_attended, cnn_pooled], dim=1)

        # ========== CLASSIFICATION ==========
        # Final classification using combined features
...
def train_model(model, train_loader, val_loader, device, n_epochs=50):
    ...
    for epoch in range(n_epochs):
        ...
        # Learning rate scheduling (now correctly using mode='max' for accuracy)
        scheduler.step(val_acc)

        # Early stopping
        if val_acc > best_val_acc:
            best_val_acc = val_acc
            torch.save(model.state_dict(), 'best_parallel_lstm_cnn.pth')
            patience_counter = 0
        else:
            patience_counter += 1
...
def plot_results(train_losses, val_losses, train_accs, val_accs, all_preds, all_labels, class_names):
    ...
    plt.tight_layout()
    plt.savefig('parallel_lstm_cnn_results.png', dpi=300, bbox_inches='tight')
    plt.show()

    print("Results saved to 'parallel_lstm_cnn_results.png'")

def main():
    """Main training pipeline"""
    print("=" * 50)
    print("Parallel LSTM-CNN Hybrid Model for Plasma Classification")
    print("=" * 50)
    ...
    # Create model
    model = ParallelLSTMCNN(n_features=len(features), n_classes=4).to(device)
    ...
    # Load best model
    print("\nLoading best model...")
    model.load_state_dict(torch.load('best_parallel_lstm_cnn.pth'))
