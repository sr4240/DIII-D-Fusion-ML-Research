...
class PlasmaDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # For NN, we flatten the window data instead of transposing
        return torch.FloatTensor(self.windows[idx].flatten()), torch.LongTensor([self.window_labels[idx]]).squeeze()
...
class PlasmaWindowDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # For NN, we flatten the window data
        return torch.FloatTensor(self.windows[idx].flatten()), torch.LongTensor([self.labels[idx]]).squeeze()

class PlasmaFourStateNNBiLSTMAttention(nn.Module):
    def __init__(self, n_features, window_size=150, n_classes=4):
        super().__init__()

        # Calculate input size: n_features * window_size
        input_size = n_features * window_size  # 1050

        # Regular neural network layers instead of CNN
        # Using LayerNorm instead of BatchNorm to avoid issues with batch size = 1
        ...
        # We'll reshape the 128-dim output to be sequence-like for LSTM
        # Create 16 timesteps with 8 features each (16 * 8 = 128)
        self.lstm_timesteps = 16
        ...
        # Direct classifier without intermediate layer
        self.classifier = nn.Linear(self.lstm_hidden_size * 2, n_classes)

        print(f"Total parameters: {sum(p.numel() for p in self.parameters()):,}")

    def forward(self, x):
        # x shape: (batch_size, n_features * window_size)
        x = self.feature_extractor(x)  # (batch_size, 128)

        # Reshape for LSTM: (batch_size, timesteps, features)
        batch_size = x.size(0)
        x = x.view(batch_size, self.lstm_timesteps, self.lstm_input_size)  # (batch_size, 16, 8)

        x, _ = self.lstm(x)      # (batch_size, 16, 32)
        x = self.attention(x)    # (batch_size, 32)
        x = self.classifier(x)   # (batch_size, 4)
        return x

class PlasmaFourStateSimpleNN(nn.Module):
    """A simpler version without LSTM for comparison"""
    def __init__(self, n_features, window_size=150, n_classes=4):
        super().__init__()

        input_size = n_features * window_size  # 1050

        # Reduced architecture to ~20k parameters
        # Parameter calculation:
        # Linear(1050 -> 18): 1050 * 18 + 18 = 18,918 params
        # Linear(18 -> 12): 18 * 12 + 12 = 228 params
        # Linear(12 -> 8): 12 * 8 + 8 = 104 params
        # Linear(8 -> 4): 8 * 4 + 4 = 36 params
        # Total: ~19,286 parameters
...
def train_and_evaluate(train_loader, val_loader, test_loader, n_features, use_lstm=True, window_size=150):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Using device: {device}")

    # Model setup - choose between LSTM version or simple NN
    if use_lstm:
        print("Using NN with BiLSTM and Attention")
        model = PlasmaFourStateNNBiLSTMAttention(n_features=n_features, window_size=window_size, n_classes=4).to(device)
        model_save_name = 'best_plasma_fourstate_nn_lstm_centerlabel.pth'
    else:
        print("Using Simple NN")
        model = PlasmaFourStateSimpleNN(n_features=n_features, window_size=window_size, n_classes=4).to(device)
        model_save_name = 'best_plasma_fourstate_nn_simple_centerlabel.pth'
    ...
    suffix = 'nn_lstm' if use_lstm else 'nn_simple'
    plt.title(f'Normalized Confusion Matrix (Four-State NN Classification)')
    plt.ylabel('True Label')
    plt.xlabel('Predicted Label')
    plt.savefig(f'confusion_matrix_fourstate_{suffix}_centerlabel.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.title(f'ROC Curves (Four-State NN Classification)')
    plt.legend()
    plt.savefig(f'roc_curves_fourstate_{suffix}_centerlabel.png', dpi=300, bbox_inches='tight')
    plt.show()

def main():
    print("=== Plasma Four-State Classification NN (Center Label, Random Split) ===")

    # Choose which model to use
    use_lstm = True  # Set to False to use the simple NN without LSTM

    X, y, shots, selected_features, scaler = load_and_prepare_data()
    train_loader, val_loader, test_loader = create_data_loaders(X, y, shots, window_size=150)
