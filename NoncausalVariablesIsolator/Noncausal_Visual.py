...
def load_and_prepare_data():
    """
    Load and prepare the dataset from noncausal_database.csv
    """
    print("=== Loading Noncausal Dataset from CSV ===")
    df = pd.read_csv('noncausal_database.csv')
...
def select_features_and_clean(df):
    """
    Select noncausal features and clean the dataset for binary classification (ELMing vs To be suppressed)
    """
    print("\n=== Feature Selection and Data Cleaning (Noncausal) ===")
    ...
    df_cleaned = df.dropna(subset=available_features, how='any').copy()
    
    # Map labels to binary classes: 1=ELMing, 0=To be suppressed
    ...
    binary_state_names = {0: 'To be suppressed', 1: 'ELMing'}
    
    print(f"\nBinary state distribution:")
    for state, count in binary_state_counts.items():
        print(f"  {binary_state_names[state]} (class {state}): {count:6d} records ({count/len(df_cleaned)*100:.1f}%)")
    
    return df_cleaned, available_features, binary_state_names

def prepare_random_splits(df_cleaned, available_features):
    """
    Prepare training and testing data with random split (80% training, 20% testing)
    """
    print("\n=== Data Splitting (Random Split) ===")
    
    # Prepare input (X) and target (y)
    X = df_cleaned[available_features]
    y = df_cleaned['binary_state']
    
    # Use train_test_split for random splitting with random_state=42 for reproducibility
    X_train, X_test, y_train, y_test = train_test_split(
        X, y, test_size=0.0001, random_state=42, stratify=y
    )
    
    print(f"Training set size: {len(X_train)} (80%)")
    print(f"Testing set size: {len(X_test)} (20%)")
    print(f"Random split with random_state=42 for reproducibility")
...
def scan_parameter_with_adaptive_ranges(clf, available_features, scenario_name, scenario_params, 
    ...
    # Vectorized scan: build matrix of all parameter values and predict once
    X_scan = []
...
def test_adaptive_parameter_scanning(clf, available_features, df_cleaned):
    ...
    # Test all parameters for comprehensive analysis
    ...
    for param_name, separation_score in all_params:
        ...
        # Always generate a plot for this feature
        plt.figure(figsize=(12, 8))
        line_styles = ['-', '--', '-.', ':']
        ...
        if param_name == 'n_eped':
            ...
            plt.title(f'{param_name}-Dependent Suppression Probability', fontsize=16, fontweight='bold')
        ...
        os.makedirs(OUT_DIR, exist_ok=True)
        out_file = os.path.join(OUT_DIR, f'notable_{param_name}_differences.png')
...
def main():
    """
    Noncausal visualization using only the noncausal feature set and RF from Noncausal_Random_Forest.py
    """
    print("=== Noncausal Plasma Parameter Visualization ===")
    ...
    print("\n" + "="*80)
    print("RUNNING ADVANCED ANALYSIS (Noncausal)")
    print("="*80)
