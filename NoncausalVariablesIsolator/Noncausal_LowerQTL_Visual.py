...
OUT_DIR = 'noncausal_lowerqtl_images'
...
def perform_cluster_analysis(df_cleaned, available_features):
    ...
    if notable_clusters:
        ...
        plt.title('Cluster Analysis of Plasma States (iln3iamp @ 25th in sweeps)', fontsize=16, fontweight='bold')
...
def test_adaptive_parameter_scanning(clf, available_features, df_cleaned):
    print("\n" + "="*80)
    print("ADAPTIVE PARAMETER SCANNING (iln3iamp fixed at 25th percentile)")
    print("="*80)
    # Build a single scenario: all params at median, ONLY iln3iamp at 25th percentile
    scenario_params = {}
    for param in available_features:
        scenario_params[param] = df_cleaned[param].median()
    if 'iln3iamp' in available_features:
        scenario_params['iln3iamp'] = df_cleaned['iln3iamp'].quantile(0.25)
    scenarios = {'iln3iamp_lower_quartile': scenario_params}
    ...
    for param_name, separation_score in all_params:
        # Ensure ONLY iln3iamp is fixed at its lower quartile during sweeps; skip sweeping iln3iamp itself
        ...
        plt.figure(figsize=(12, 8))
        label = 'Only iln3iamp at 25th percentile'
        ...
        if param_name == 'n_eped':
            ...
            plt.title('Suppression Probability of Electron Density (iln3iamp 25th)', fontsize=16, fontweight='bold')
        else:
            plt.xlabel(f'{param_name}', fontsize=14)
            plt.title(f'{param_name}-Dependent Suppression Probability (iln3iamp 25th)', fontsize=16, fontweight='bold')
        plt.ylabel('Suppression Probability', fontsize=14)
        plt.legend(fontsize=12)
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        os.makedirs(OUT_DIR, exist_ok=True)
        out_file = os.path.join(OUT_DIR, f'lowqtl_{param_name}_sweep.png')
...
def main():
    print("=== Noncausal Plasma Parameter Visualization (iln3iamp at lower quartile during sweeps) ===")
    ...
    print("\n" + "="*80)
    print("RUNNING ANALYSIS WITH iln3iamp FIXED AT 25th PERCENTILE")
    print("="*80)
    all_notable_findings = []
    notable_params, separation_scores = analyze_parameter_distributions(df_cleaned, available_features)
    if notable_params:
    ...
    print("\n" + "="*80)
    print("ANALYSIS SUMMARY (iln3iamp @ 25th during sweeps)")
    print("="*80)
