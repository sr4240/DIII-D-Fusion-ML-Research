...
class PlasmaDataset(Dataset):
    def __init__(self, features, labels, shots, window_size=30):
    ...
    def _create_windows(self):
        ...
        for shot_id in np.unique(self.shots):
            ...
            for i in range(start_idx, end_idx - self.window_size + 2):
                ...
                window_label = Counter(window_labels_slice).most_common(1)[0][0]
                label_counts = Counter(window_labels_slice)
                if (max(label_counts.values()) >= self.window_size * 0.7) and (window_label != 0):
...
def train_and_evaluate(train_loader, val_loader, test_loader, n_features):
    ...
    for epoch in range(n_epochs):
        ...
        if val_f1 > best_val_f1:
            best_val_f1 = val_f1
            torch.save(model.state_dict(), 'best_plasma_binary_cnn.pth')
            patience_counter = 0
        else:
            patience_counter += 1
    ...
    model.load_state_dict(torch.load('best_plasma_binary_cnn.pth'))
    model.eval()
...
def main():
    print("=== Plasma Binary State Classification CNN ===")
