...
class BinaryFocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        # inputs: (batch_size, 1) - raw logits
        # targets: (batch_size,) - binary labels (0 or 1)
        bce_loss = nn.BCEWithLogitsLoss(reduction='none')(inputs.squeeze(), targets.float())
...
class BinaryPlasmaDataset(Dataset):
    def __init__(self, features, labels, window_size=150):
        ...
        # Binary classification: ELM (4) -> 1, others (1,2,3) -> 0 (Suppressed)
        binary_labels = []
        for label in self.window_labels:
            if label == 4:  # ELM
                binary_labels.append(1)
            else:  # Suppressed (combines Dithering, Mitigated, and original Suppressed)
                binary_labels.append(0)
        
        self.window_labels = np.array(binary_labels)
        
        print(f"Created {len(self.windows)} binary TCN windows with window size {window_size}")
        print(f"Binary label distribution: Suppressed (0): {np.sum(self.window_labels == 0)}, ELM (1): {np.sum(self.window_labels == 1)}")
    ...
    def __getitem__(self, idx):
        ...
        label = torch.FloatTensor([self.window_labels[idx]])  # Float for binary classification
        return features, label.squeeze()

class BinaryPlasmaTCN(nn.Module):
    def __init__(self, n_features, window_size=150, num_channels=[64, 128, 256]):
        super(BinaryPlasmaTCN, self).__init__()
        ...
        # Binary classifier (single output for ELM vs Suppressed)
    ...
    def forward(self, x):
        ...
        # Binary classification (outputs logits)
...
def train_model(X_train, y_train, X_test, y_test, n_features, window_size=150):
    print("Creating binary TCN datasets...")
    train_dataset = BinaryPlasmaDataset(X_train, y_train, window_size)
    test_dataset = BinaryPlasmaDataset(X_test, y_test, window_size)
    ...
    # Use Binary Focal Loss for better handling of class imbalance
    criterion = BinaryFocalLoss(alpha=1, gamma=2)

    model = BinaryPlasmaTCN(n_features=n_features, window_size=window_size).to(device)
    ...
    print("Starting binary TCN training...")
    for epoch in range(n_epochs):
        ...
        if accuracy > best_accuracy:
            ...
            # torch.save(model.state_dict(), 'best_binary_plasma_tcn.pth')  # Commented out file saving
            patience_counter = 0
        else:
            patience_counter += 1
    ...
    # model.load_state_dict(torch.load('best_binary_plasma_tcn.pth'))  # Commented out file loading
    model.eval()
    all_predictions = []
    all_labels = []
    all_probabilities = []
    ...
    plt.title(f'Binary TCN Confusion Matrix\nAccuracy: {accuracy:.4f}, AUC: {auc_score:.4f}')
    plt.ylabel('True Label')
    plt.xlabel('Predicted Label')
    plt.tight_layout()
    # plt.savefig('binary_tcn_confusion_matrix.png', dpi=300, bbox_inches='tight')  # Commented out file saving
    plt.show()
    ...
    plt.title('Binary TCN ROC Curve')
    plt.legend(loc="lower right")
    plt.grid(True)
    plt.tight_layout()
    # plt.savefig('binary_tcn_roc_curve.png', dpi=300, bbox_inches='tight')  # Commented out file saving
    plt.show()
    ...
    plt.title('Binary TCN Training and Test Loss')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend()
    plt.subplot(1, 2, 2)
    plt.plot(test_losses, label='Test Loss')
    plt.title('Binary TCN Test Loss')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend()
    plt.tight_layout()
    # plt.savefig('binary_tcn_training_curves.png', dpi=300, bbox_inches='tight')  # Commented out file saving
    plt.show()
    
    return model

def main():
    print("=== Binary Plasma State Classification TCN ===")
    print("Classifying: ELM vs Suppressed (Dithering + Mitigated + Suppressed)")
    ...
    print("\n=== Binary TCN Training Complete ===")
    # print("Model saved as 'best_binary_plasma_tcn.pth'")  # Commented out file saving message
    # print("Confusion matrix, ROC curve, and training curves saved as images")  # Commented out file saving message
