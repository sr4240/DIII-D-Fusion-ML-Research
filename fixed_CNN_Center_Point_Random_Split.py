...
class PlasmaDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # FIXED: Proper data shape for Conv1d
        # windows[idx] shape: (window_size, n_features)
        # Conv1d expects: (n_features, window_size)
        window = self.windows[idx]  # Shape: (window_size, n_features)
        # Transpose to get (n_features, window_size) for Conv1d
        window_tensor = torch.FloatTensor(window.T)  # Now: (n_features, window_size)
...
class PlasmaFourStateCNNBiLSTMAttention(nn.Module):
    def __init__(self, n_features, n_classes=4):
        super().__init__()
        # FIXED: Changed stride from 2 to 1 to preserve temporal resolution
        self.feature_conv = nn.Sequential(
            nn.Conv1d(n_features, 16, kernel_size=5, stride=1, padding=2),
        ...
        # For window_size=150, with stride=1: 150 -> 150 -> 150 (preserved)
    ...
    def forward(self, x):
        # FIXED: Data flow with corrected shapes
        # x shape: (batch_size, n_features, window_size)
        x = self.feature_conv(x)  # (batch_size, 32, window_size) - stride=1 preserves size
        x = x.permute(0, 2, 1)   # (batch_size, window_size, 32)
        x, _ = self.lstm(x)      # (batch_size, window_size, 32) - bidirectional
...
def train_and_evaluate(train_loader, val_loader, test_loader, n_features):
    ...
    if val_losses:
        ...
        ax1.axvline(x=best_epoch, color='green', linestyle='--', alpha=0.5, label=f'Best Val Loss (Epoch {best_epoch})')
    ...
    if val_accuracies:
        ...
        ax2.axvline(x=best_acc_epoch, color='green', linestyle='--', alpha=0.5, label=f'Best Val Acc (Epoch {best_acc_epoch})')
        ax2.scatter(best_acc_epoch, val_accuracies[best_acc_epoch-1], color='green', s=100, zorder=5)

    plt.suptitle('CNN Training History - Fixed Data Shape & Stride=1', fontsize=16, fontweight='bold', y=1.02)
    plt.tight_layout()
